...
ActionsClones Auto::Solve(const std::string& task, const std::string& task_name,
  ...
  if (task_name == "ext") {
    ...
    futures.emplace_back(tp->enqueueTask<Result>(
        std::make_shared<std::packaged_task<Result()>>([&]() {
          BaseClones1Settings sett{-1, 0, 200, true, true, true, true, true};
          BaseClones1 bc;
          return Result("bc_mops2_fw", bc.Solve(task, sett, bonuses));
        })));
    ...
    */
    ...
    //     BaseClones1Settings sett{-1, i & 1, j, i & 2, true, i & 4, i & 8, i &
    //     16}; BaseClones1 bc; m.AddSolution(
    ...
    TaskSplitter<clone::Greedy1, worker::Local> tsp_greedy_local(clone_greedy1,
                                                                 worker_local);
    TaskSplitter<clone::Greedy1, worker::Local2> tsp_greedy_local2(
        clone_greedy1, worker_local2);
    ...
    m.AddSolution(tsp_greedy_local2.Solve(task, bonuses), "tsp_g2");
  }
  return m.Solution();
}
}  // namespace solvers
